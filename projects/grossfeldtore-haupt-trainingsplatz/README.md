# Großfeldtore Haupt- und Trainingsplatz

## Purpose

Dieses Projekt bündelt die Umstellung der Großfeldtore im TuS Sportpark:

- die beiden vorhandenen mobilen Großfeldtore des Hauptfelds sollen künftig auf dem Trainingsfeld genutzt werden,
- dadurch sollen auf dem Trainingsfeld insgesamt vier große Fußballtore zur Verfügung stehen,
- auf dem Hauptfeld sollen zwei neue Großfeldtore 7,32 × 2,44 m in Bodenhülsen installiert werden,
- die neuen Tore müssen vollständig aus den Hülsen herausnehmbar sein, damit intensive Rasenpflege und insbesondere Rasennähen ohne störende Toranlage möglich bleiben.

Das Projekt umfasst Auswahl, Förderprüfung, Beschluss, Beschaffung, Einbau der Bodenhülsen und Inbetriebnahme der neuen Toranlage.

## Core Principle

> **Mehr Trainingsqualität, ohne die Pflegequalität des Hauptfelds einzuschränken.**

Die neue Toranlage soll im normalen Spielbetrieb stabil und pflegeleicht sein. Gleichzeitig muss sie für größere Rasenarbeiten mit vertretbarem Aufwand vollständig zurückgebaut werden können.

Deshalb wird aktuell eine Lösung mit Hülsentoren, P-Netzbügeln und hochklappbarem Bodenrahmen bevorzugt. Separate Spannpfosten und Spannseile hinter den Toren sollen vermieden werden.

## Main Content

### 1. Zielzustand

#### Trainingsfeld

- 4 mobile Großfeldtore insgesamt,
- davon 2 bereits vorhandene mobile Tore, die vom Hauptfeld umgesetzt werden.

#### Hauptfeld

- 2 neue Großfeldtore 7,32 × 2,44 m,
- je Tor 2 Bodenhülsen,
- insgesamt 4 Bodenhülsen,
- P-Netzbügel oben,
- hochklappbarer Bodenrahmen unten,
- keine separaten Spannpfosten hinter dem Tor,
- komplettes Tor für Rasenregeneration/Rasennähen aus den Bodenhülsen entnehmbar.

### 2. Aktuell favorisierte Lösung

Aktuell favorisiert wird die Kombination von Kübler Sport:

#### Kübler Sport® Fußballtor STADION, eckverschweißt, P-Netzbügel

Produktlink:

`https://www.kuebler-sport.de/fussballtor-in-bodenhuelsen-mit-p-netzbuegel-config-f5400.html?sku=F5455`

Aktueller Hersteller-/Shopstand vom 05.09.2026:

- 7,32 × 2,44 m,
- Aluminium,
- Torrahmen 120 × 100 mm,
- P-Netzbügel Ø 40 mm,
- obere Tortiefe 80 cm,
- Netzbefestigung POWER PRESS,
- 2 Bodenhülsen mit Abdeckkappen je Tor enthalten,
- Einstecktiefe 500 mm,
- sicherheitsgeprüft,
- DIN EN 748,
- nach Angaben des Anbieters entsprechend DFB-/FIFA-Bestimmungen,
- Tornetz nicht enthalten,
- Bodenrahmen nicht enthalten.

Preisstand 05.09.2026:

- 1.119,00 € brutto je Tor,
- 2 Tore = 2.238,00 € brutto.

#### Kübler Sport Bodenrahmen STADION FLEX, Artikel F1074

Produktlink:

`https://www.kuebler-sport.de/kuebler-sport-bodenrahmen-stadion-flex-f1074.html`

Aktueller Hersteller-/Shopstand vom 05.09.2026:

- Preis pro Paar regulär 719,00 € brutto,
- aktuell ausgewiesener Sonderpreis 697,50 € brutto pro Paar,
- Sonderpreis laut Anbieter zeitlich befristet und nur bei Direktbestellung im Onlineshop, solange Vorrat reicht,
- hochklappbar,
- Aluminium-Rundrohr Ø 35 mm,
- untere Tortiefe von 1,50 bis 2,00 m verstellbar,
- Netzbefestigung am Bodenrahmen nicht integriert; empfohlen werden Netzringe oder Leine,
- aktuell längere Lieferzeit,
- Frachtkosten vor Bestellung für die konkrete Gesamtbestellung bestätigen.

### 3. Aktueller Kostenstand

| Position | Menge | Bruttopreis |
|---|---:|---:|
| Kübler Sport STADION mit P-Netzbügel | 2 | 2.238,00 € |
| 4 Bodenhülsen inkl. Abdeckkappen | 4 | in den Torpreisen enthalten |
| Kübler Sport STADION FLEX Bodenrahmen | 1 Paar / 2 Stück | 697,50 € Sonderpreis |
| **Bekannter Hardware-Zwischenstand** | | **2.935,50 € brutto** |

Rechnerischer Nettowert des bekannten Hardware-Zwischenstands bei 19 % MwSt.:

**ca. 2.466,81 € netto**

Noch nicht im belastbaren Gesamtpreis enthalten:

- 2 passende Tornetze,
- Netzringe oder Leine für den Bodenrahmen,
- endgültige Fracht-/Versandkosten,
- Beton/Fundamente,
- Einbau der Bodenhülsen,
- gegebenenfalls Montage-/Serviceleistungen,
- gegebenenfalls Pfostenmanschetten oder weiteres Zubehör.

Die vollständige Beschaffungsgrundlage steht in `BESTELLGRUNDLAGE.md`.

### 4. Förderbezug

Für das Vorhaben ist die **BSB Nord Sportgeräteförderung 2026** relevant.

Verifizierter Förderweg:

- 30 % Zuschuss auf anerkannte zuschussfähige Kosten,
- `Tore als Satz` mit bis zu 3.000 € förderfähigem Höchstbetrag,
- maximal 900 € Zuschuss,
- Anschaffung/Rechnung im Kalenderjahr 2026,
- formaler Antrag **nach der Anschaffung** mit Rechnung und Zahlungsnachweis,
- Antrag ausschließlich online im BSBnet,
- Antrag für 2026 bis 31.01.2027.

Für den aktuell bekannten Hardware-Zwischenstand von 2.935,50 € ergäbe sich bei vollständiger Anerkennung rechnerisch ein Zuschuss von **880,65 €**.

Wenn der anerkannte Torsatz inklusive weiterer förderfähiger Komponenten mindestens 3.000 € erreicht, liegt der rechnerische maximale Zuschuss bei **900,00 €**.

Vor Bestellung wird der konkrete Torsatz trotzdem schriftlich mit dem BSB Nord abgeklärt, weil noch offen ist, ob hochklappbare Bodenrahmen und Netze vollständig zum Satz zählen und weil die Richtlinie die Sportgeräte-Grundausstattung gemeindeeigener Sportfreianlagen ausschließt.

Die vollständige projektbezogene Förderreferenz mit offiziellen Quellen, Ansprechpartner, Antragsweg und fertiger Vorabprüfungs-Anfrage steht in:

`FUNDING-REFERENCE.md`

Diese Rechnung ist **keine Förderzusage**.

### 5. Projektartefakte

Zentraler Google-Drive-Ordner:

`https://drive.google.com/drive/folders/1_CVr1G6wScpbpBagyGTwF99iJFQ0IiYT`

Vorstandsfertige Entscheidungsunterlage:

`https://docs.google.com/document/d/1HjCoYt1AwrlCBQuzxjn3zsE79YvGUWlv5vpLTjHL-2E/edit`

Dort sollen künftig insbesondere liegen:

- Vorstandsvorlage,
- Vergleichsangebote / Warenkorb,
- Produkt-/Montageunterlagen,
- BSB-Nord-Vorabprüfung,
- Fundamentpläne,
- gegebenenfalls Fotos und Aufmaß der bestehenden Situation,
- Beschluss-/Bestellunterlagen,
- Rechnung, Zahlungsnachweis und Förderbelege.

GitHub hält Projektzustand, Beschaffungsgrundlage, Förderreferenz, Entscheidungen und Links. Binäre Unterlagen bleiben im Drive-Artefaktraum.

### 6. Bewusste Grenzen

Noch nicht entschieden oder noch nicht vollständig geklärt sind:

- finale Bestellung,
- genaue Torfarbe/Variante,
- konkrete Tornetze,
- finale Netztiefe 1,50 / 2,00 m,
- Art und Menge der Netzringe bzw. Leine,
- endgültige Versandkosten,
- Fundamentierung und Einbaukosten,
- ausführender Einbaupartner,
- fachlicher Projekt-Owner,
- Eigentums-/Nutzungsstatus des Hauptplatzes für die Förderung,
- schriftliche BSB-Nord-Bestätigung zur konkreten Förderfähigkeit,
- finaler Vorstandsbeschluss / Budgetrahmen.

Eine Bestellung wurde durch dieses Projekt **nicht** ausgelöst.

## Relationship to other documents

- `PROJECT-STATE.md`
- `BESTELLGRUNDLAGE.md`
- `FUNDING-REFERENCE.md`
- `../PROJECT-PORTFOLIO.md`
- `../../knowledge/funding/FUNDING-RADAR.md`
- `../../knowledge/funding/FUNDING-CALENDAR.md`
- `../../roles/funding-grants-manager/role.md`
- `../../roles/project-portfolio-manager/role.md`

## Future Development

Der nächste Reifegewinn ist ein belastbarer, förderkonformer Gesamtpreis.

Dafür werden als Nächstes:

1. Vorstandsbeschluss und Budgetrahmen klären,
2. BSB-Nord-Vorabprüfung der konkreten Kombination durchführen,
3. Eigentums-/Nutzungsstatus des Hauptplatzes klären,
4. passende Netze für 7,32 × 2,44 m und die gewählte untere Netztiefe festlegen,
5. Netzringe/Leine ergänzen,
6. Frachtkosten verbindlich ermitteln,
7. Fundament-/Einbauaufwand und Standortmaße klären,
8. Gesamtbudget aktualisieren,
9. Bestellung auslösen,
10. anschließend BSBnet-Förderantrag mit Rechnung und Zahlungsnachweis stellen.
