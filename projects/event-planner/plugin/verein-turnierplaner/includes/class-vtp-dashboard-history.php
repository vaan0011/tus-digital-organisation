 ...
 public static function render(){
  ...
  if(!$selected_year){
   ...
   foreach(array_slice($details,0,12) as $row){
    ...
    echo '<a class="vtp-dashboard-list-item" href="'.esc_url($url).'"><span class="vtp-dashboard-type">'.esc_html($type).'</span><span class="vtp-dashboard-main"><strong>'.esc_html($row->name).'</strong><small>'.esc_html(VTP_Dashboard::format_date_for_history($row->start_date,$row->end_date)).($row->location?' · '.esc_html($row->location):'').'</small></span><span aria-hidden="true">›</span></a>';
   }
   echo '</div>';
  }
