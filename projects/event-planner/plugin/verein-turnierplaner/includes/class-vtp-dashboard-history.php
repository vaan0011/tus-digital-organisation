...
class VTP_Dashboard_History {
 ...
 public static function render(){
  ...
  $event_year_rows=$has_event_type
   ? $wpdb->get_results("SELECT YEAR(start_date) AS y, COALESCE(event_type,'event') AS dashboard_type, COUNT(*) AS amount FROM $events_table WHERE status='archiviert' AND start_date IS NOT NULL AND start_date<>'' GROUP BY YEAR(start_date), dashboard_type ORDER BY y DESC")
   : $wpdb->get_results("SELECT YEAR(start_date) AS y, 'event' AS dashboard_type, COUNT(*) AS amount FROM $events_table WHERE status='archiviert' AND start_date IS NOT NULL AND start_date<>'' GROUP BY YEAR(start_date) ORDER BY y DESC");
  ...
  if($selected_year && !isset($years[$selected_year])) $selected_year=(int)array_key_first($years);
  ...
  if($selected_year){
   ...
   usort($details,function($a,$b){
    $ad=$a->start_date?:'0000-00-00';
    $bd=$b->start_date?:'0000-00-00';
  ...
  if(!$selected_year){
   ...
   foreach(array_slice($details,0,12) as $row){
    ...
    $url=$row->dashboard_type==='tournament'
     ? add_query_arg(['page'=>'vtp','edit'=>$row->id,'archive'=>1],admin_url('admin.php'))
     : add_query_arg(['page'=>'vtp-events','edit_event'=>$row->id,'archive'=>1],admin_url('admin.php'));
    echo '<a class="vtp-dashboard-list-item" href="'.esc_url($url).'">';
    echo '<span class="vtp-dashboard-type">'.esc_html($type).'</span>';
    echo '<span class="vtp-dashboard-main"><strong>'.esc_html($row->name).'</strong><small>'.esc_html(self::format_date($row->start_date,$row->end_date)).($row->location?' · '.esc_html($row->location):'').'</small></span>';
