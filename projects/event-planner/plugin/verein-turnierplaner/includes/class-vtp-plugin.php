...
public function print_pdf(){
 ...
 private function manage_event($ev){ global $wpdb;
  ...
  $savedDays=get_option('vtp_event_days_'.absint($ev->id),[]); if(is_array($savedDays)){ foreach($savedDays as $savedDay){ if(self::is_valid_event_date($savedDay)) $days[$savedDay]=[]; } }
  ...
  foreach($days as $date=>$program){ $dayIndex++; echo '<div class="vtp-day-card"><div class="vtp-day-head"><h3>Tag '.$dayIndex.' – '.esc_html($date?date_i18n('l, d. F Y',strtotime($date)):'neuer Tag').'</h3><p class="vtp-day-actions"><label>Datum <input type="date" class="vtp-day-date" name="event_day[]" value="'.esc_attr($date).'" required></label> <button type="button" class="button vtp-toggle-day">Einklappen</button> <button type="button" class="button vtp-duplicate-day">Tag duplizieren</button> <button type="button" class="button vtp-add-program">Programmpunkt hinzufügen</button> <button type="button" class="button vtp-remove-day">Tag entfernen</button></p></div><div class="vtp-day-body"><div class="vtp-program-rows">';
    foreach($program as $it){
      ...
      $itemVisibility=in_array($it['type'],['Aufbau','Abbau'],true)?'private':($it['visibility']??'public');
      echo '<div class="vtp-program-row"><input type="time" name="start_time[]" value="'.esc_attr(substr((string)$it['time'],0,5)).'" aria-label="Start"><input type="time" name="end_time[]" value="'.esc_attr(substr((string)$it['end'],0,5)).'" aria-label="Ende"><select class="vtp-item-type" name="item_type[]"><option '.selected($it['type'],'Aufbau',false).'>Aufbau</option><option '.selected($it['type'],'Abbau',false).'>Abbau</option><option '.selected($it['type'],'Programmpunkt',false).'>Programmpunkt</option><option '.selected($it['type'],'Musik',false).'>Musik</option><option '.selected($it['type'],'Spiel',false).'>Spiel</option></select><input type="text" name="title[]" value="'.esc_attr($it['title']).'" placeholder="Titel" required><select class="vtp-item-visibility" name="visibility[]" aria-label="Sichtbarkeit"><option value="public" '.selected($itemVisibility,'public',false).'>öffentlich</option><option value="private" '.selected($itemVisibility,'private',false).'>nicht öffentlich</option><option value="ticket" '.selected($itemVisibility,'ticket',false).'>Eintrittskarte</option><option value="members" '.selected($itemVisibility,'members',false).'>Mitglieder</option></select><input type="hidden" name="item_date[]" value="'.esc_attr($date).'"><button type="button" class="vtp-icon-btn vtp-remove-program" title="Programmpunkt entfernen">✕</button></div>';
    }
    echo '</div></div></div>'; }
 ...
 public static function is_valid_event_date($date){
  if(!is_string($date) || !preg_match('/^(\d{4})-(\d{2})-(\d{2})$/',$date,$parts)) return false;
  return checkdate((int)$parts[2],(int)$parts[3],(int)$parts[1]);
 }
 public static function normalize_event_item($type,$title,$visibility){
  $allowedTypes=['Aufbau','Abbau','Programmpunkt','Musik','Spiel'];
  if(!in_array($type,$allowedTypes,true)) $type='Programmpunkt';
  $title=trim((string)$title); if($title==='') $title=$type;
  $allowedVisibility=['public','private','ticket','members'];
  if(!in_array($visibility,$allowedVisibility,true)) $visibility='public';
  if(in_array($type,['Aufbau','Abbau'],true)) $visibility='private';
  return ['type'=>$type,'title'=>$title,'visibility'=>$visibility];
 }
 public function save_event_items(){
  $this->verify('vtp_save_event_items'); global $wpdb; $eid=absint($_POST['event_id']);
  $eventDays=[]; foreach((array)($_POST['event_day']??[]) as $day){ $day=sanitize_text_field($day); if(self::is_valid_event_date($day)) $eventDays[]=$day; }
  $eventDays=array_values(array_unique($eventDays)); sort($eventDays); update_option('vtp_event_days_'.$eid,$eventDays,false);
  ...
  if(isset($_POST['title']) && is_array($_POST['title'])){
    ...
    foreach($titles as $idx=>$title){ $date=sanitize_text_field($dates[$idx]??''); if(!self::is_valid_event_date($date)) continue;
      $item=self::normalize_event_item(sanitize_text_field($types[$idx]??'Programmpunkt'),sanitize_text_field($title),sanitize_key($visibilities[$idx]??'public'));
      $wpdb->insert(VTP_DB::table('event_items'),['event_id'=>$eid,'item_date'=>$date,'start_time'=>sanitize_text_field($starts[$idx]??''),'end_time'=>sanitize_text_field($ends[$idx]??''),'item_type'=>$item['type'],'title'=>$item['title'],'visibility'=>$item['visibility'],'sort_order'=>$i++]);
    }
  } else {
