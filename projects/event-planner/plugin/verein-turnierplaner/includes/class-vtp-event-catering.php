...
class VTP_Event_Catering {
 ...
 private static function ensure_schema(){
  if(get_option('vtp_event_catering_schema')===self::SCHEMA_VERSION) return;
  global $wpdb;
  require_once ABSPATH.'wp-admin/includes/upgrade.php';
  $c=$wpdb->get_charset_collate();
  dbDelta("CREATE TABLE ".VTP_DB::table('event_catering_items')." (
 ...
 public static function assets($hook){
  ...
  $rows=$wpdb->get_results($wpdb->prepare(
   "SELECT id,category,item_name,quantity,unit,note,sort_order
 ...
 public static function save(){
  ...
  foreach($names as $i=>$raw_name){
   ...
   if($name==='' && $raw_quantity==='' && $unit==='' && $note==='') continue;
   if(!in_array($category,['drink','food'],true)) wp_die('Ungültige Bewirtungskategorie.');
   if($name==='') wp_die('Bitte für jeden Bewirtungseintrag einen Artikel angeben.');
   if(!is_numeric($raw_quantity) || (float)$raw_quantity<=0) wp_die('Bitte für jeden Bewirtungseintrag eine Bestellmenge größer 0 angeben.');
